...
rightChSuffix = "FRET"; // Suffix to be added to saved file name
		...
		for (k = 0; k < lengthOf(fileList); k++) {
			...
			if (endsWith(fileList[k], "posZ0.tif")) {
				...
				if (leftCh == true) {
					...
					run("Bio-Formats Importer", "open=[" + rawDir + "/" + expList[i] + "/" + posList[j] + "/" + fileList[k] + "] color_mode=Default crop rois_import=[ROI manager] specify_range view=Hyperstack stack_order=XYCZT t_begin=" + leftChStart + " t_end=" + leftChEnd + " t_step=1 x_coordinate_1=0 y_coordinate_1=0 width_1=427 height_1=684");
					rename(title);
				...
				if (rightCh == true) {
					...
					run("Bio-Formats Importer", "open=[" + rawDir + "/" + expList[i] + "/" + posList[j] + "/" + fileList[k] + "] color_mode=Default crop rois_import=[ROI manager] specify_range view=Hyperstack stack_order=XYCZT t_begin=" + rightChStart + " t_end=" + rightChEnd + " t_step=1 x_coordinate_1=427 y_coordinate_1=0 width_1=427 height_1=684");
					rename(title);
					Stack.getDimensions(width, height, channels, slices, frames);
